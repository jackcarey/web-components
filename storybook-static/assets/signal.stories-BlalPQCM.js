import r from"./index-jeR3H6yP.js";import{x as v}from"./lit-element-B-NARBzP.js";const D={},a=(o,t,e)=>{const s=`#${`signal-${Math.floor(Math.random()*1e3)}`}`;return v`<div id="${s}"><p>
            Story '<code>${o.name}</code>' | If the DOM does not update below, check the
            browser console.
        </p>
        ${e?v`<p>${e}</p>`:""}
        <pre><output>${t?JSON.stringify(t,null,2):""}<output></pre></div>`},i={decorators:[(o,t)=>{const e=new r({foo:"bar"});return e.addEventListener("signal",n=>console.log("listener on signal",n)),e.fizz="bang",e.bang="fizz",a(t,e)}]},l={decorators:[(o,t)=>{document.addEventListener("signal",n=>console.log("document story signal",t.name,n));const e=new r({foo:"bar"},{target:document});return e.addEventListener("signal",n=>console.log("listener on signal",n)),e.fizz="bang",e.bang="fizz",a(t,e,"This story adds the 'document' as the EventTarget for the Signal")}]},c={decorators:[(o,t)=>{const e=new r({},{target:document}),n=s=>console.log("listener on signal",s);e.addEventListener("signal",n);for(let s=0;s<1e3;++s)e[s]=Math.round(Math.random()*1e4);return a(t,e)}]},d={decorators:[(o,t)=>{const e=new r({});e.addEventListener("signal",m=>console.log("listener on signal",m));let n=0;const s=setInterval(()=>{e[n]=performance.now(),n+=1},500);return setTimeout(()=>{clearInterval(s)},1e4),a(t,e,"New events are triggered every 500ms for 10 seconds. The DOM will not change.")}]},g={decorators:[(o,t)=>{const e="customName",n=new r({},{name:e,useSuffix:!0});n.addEventListener("signal",u=>console.error("This should never trigger!",u)),n.addEventListener(`signal-${e}`,u=>console.log(`listener for signal-${e}`,u));let s=0;const m=setInterval(()=>{n[s]=performance.now(),s+=1},500);return setTimeout(()=>{clearInterval(m)},3e3),a(t,n,"The 'useSuffix' option appends the signal name to the end of the emitted event type, should you wish to change it.")}]};var f,p,h;i.parameters={...i.parameters,docs:{...(f=i.parameters)==null?void 0:f.docs,source:{originalSource:`{
...
}`,...($=(_=g.parameters)==null?void 0:_.docs)==null?void 0:$.source}}};const O=["Default","WithDocumentTarget","LotsOfProperties","Interval","UseSuffix"];export{i as Default,d as Interval,c as LotsOfProperties,g as UseSuffix,l as WithDocumentTarget,O as __namedExportsOrder,D as default};
//# sourceMappingURL=signal.stories-BlalPQCM.js.map
