...
if (!fs.existsSync(outputPath) || fs.readFileSync(outputPath, 'utf8') !== commitHash) {
    ...
    console.log(`Last commit hash for jsr.json files (${commitHash}) saved to ${outputPath}`);
} else {
    console.log(`Last commit hash for jsr.json files (${commitHash}) already saved to ${outputPath}`);
}
